# Temporary media-server stack for testing on Marth.
# This is intentionally a NixOS module, not a Home Manager module, because
# Jellyfin/Navidrome/rTorrent/ruTorrent/Caddy are system services.
{ config, lib, pkgs, ... }:

let
  mediaRoot = "/srv/media";

  # Temporary LAN DNS settings for Marth. When this stack moves to Roy, change
  # this to Roy's reserved DHCP/static LAN address.
  serverLanIp = "192.168.1.58";
  lanDomain = "home.arpa";
  ...
  # Leave disabled until the values below are copied from a ProtonVPN WireGuard
  # config file and the private key has been added to sops as
  ...
  protonVpn = {
    ...
    peerPublicKey = "uyhYSdZ5+7HF3LdecPicLRTN/QHPl8dvLo8VttNRT3M=";
    endpoint = "89.222.103.8:51820";
...
in
{
  ...
  # localhost for now. Configure them from Marth directly at:
  ...
  assertions = [
    {
      ...
      message = "Set protonVpn.address in hosts/marth/server_config.nix from Proton's WireGuard [Interface] Address before enabling Proton rTorrent VPN routing.";
    }
    {
      assertion = !protonVpn.enable || protonVpn.peerPublicKey != "CHANGE-ME";
      message = "Set protonVpn.peerPublicKey in hosts/marth/server_config.nix from Proton's WireGuard [Peer] PublicKey before enabling Proton rTorrent VPN routing.";
    }
    {
      assertion = !protonVpn.enable || protonVpn.endpoint != "CHANGE-ME.protonvpn.net:51820";
      message = "Set protonVpn.endpoint in hosts/marth/server_config.nix from Proton's WireGuard [Peer] Endpoint before enabling Proton rTorrent VPN routing.";
    }
  ];
